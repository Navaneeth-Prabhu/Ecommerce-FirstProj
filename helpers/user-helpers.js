...
module.exports = {
  ...
  generateAccessToken: async () => {
    const response = await fetch(base + "/v1/oauth2/token", {
      ...
      headers: {
        Authorization:
          "Basic " +
          Buffer.from(CLIENT_ID + ":" + APP_SECRET).toString("base64"),
      },
    });
    const data = await response.json();
  ...
  incrementCart: (productId, UserId, quantity) => {
    return new Promise(async (resolve, reject) => {
      let x = {
        item: ObjectID(productId),
        quantity: qua,
  ...
  cartTotal: (userId,products) => {
      
      return new Promise(async (resolve, reject) => {
        console.log("proo",products);
    //   let products = getCartProduct(userId);
        ...
        console.log(data.product.price);
              try {
                ...
                if (data.product.offerPrice) {
                    console.log(data.product.offerPrice);
